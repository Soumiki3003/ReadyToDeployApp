...
class StudentService:
    ...
    def create_student(self, item: schemas.CreateStudent) -> models.Student:
        ...
        with self.__gateway.session() as session:
            new_user = session.execute_write(tx_fn, item)
        return new_user
    ...
    def get_student(self, id: int) -> models.Student | None:
        ...
        with self.__gateway.session() as session:
            return session.execute_read(tx_fn, id)
    ...
    def get_student_by_email(self, email: str) -> models.Student | None:
        ...
        with self.__gateway.session() as session:
            return session.execute_read(tx_fn, email)
    ...
    def get_student_trajectory(self, student_id: int) -> list[models.StudentTrajectory]:
        ...
        with self.__gateway.session() as session:
            return session.execute_read(tx_fn, student_id)
    ...
    def add_trajectory_entry(
        ...
        with self.__gateway.session() as session:
            return session.execute_write(tx_fn, student_id, trajectory_entry)
    ...
    def increment_trajectory_query_repeat_count(
        ...
        with self.__gateway.session() as session:
            return session.execute_write(tx_fn, trajectory_id, increment=increment)
